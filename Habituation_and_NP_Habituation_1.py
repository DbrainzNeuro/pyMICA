...
data = data + ending
Habit_data = sorted(glob.glob(data), key=os.path.getmtime)
...
for animal in list(habit.getAnimal()):
    for phase in phases:
        ...
        visits = habit.getVisits(start = phase['Start'],end = phase['End'], mice = animal)
        if len(visits) >= 2:
            module = visits[1].Module
        ...
        data_frame.at[index,"Module"] = module
        ...
        data_frame.at[index,"Lick Time"] = licktime.microseconds / 100000          
